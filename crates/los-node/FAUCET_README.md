...
Rate-limited faucet for distributing test LOS tokens on testnet.

## Features

- **IP Rate Limiting:** 1 request per hour per IP address
- **Address Rate Limiting:** 1 request per 24 hours per LOS address
...
{
  "success": true,
  "message": "Successfully sent 100 LOS to LOS1abc123...",
...
async function requestFaucet(address) {
  ...
  if (data.success) {
    console.log(`Received ${data.amount / 100000000} LOS`);
...
async function getFaucetStats() {
  ...
  console.log(`Total distributed: ${stats.total_distributed_los} LOS`);
...
def request_faucet(address):
    ...
    if data['success']:
        print(f"Received {data['amount'] / 100000000} LOS")
...
def get_faucet_stats():
    ...
    stats = response.json()
    print(f"Total distributed: {stats['total_distributed_los']} LOS")
...
docker logs -f los-validator-1 | grep "faucet"

# Monitor rate limit hits
docker logs -f los-validator-1 | grep "rate limit exceeded"
```
...
```bash
los-cli balance <FAUCET_ADDRESS>
```

Refill from treasury:
```bash
los-cli send --from <TREASURY> --to <FAUCET_ADDRESS> --amount 10000
```
...
1. User enters LOS address
2. Solves CAPTCHA (if enabled)
3. Clicks "Request Test Tokens"
4. Receives 100 LOS after rate limit checks
...
[testnet]
faucet_enabled = true
faucet_drip_amount = 10000000000  # 100 LOS
```
...
Run faucet tests:
```bash
cargo test --package los-node --lib faucet
```

Test rate limiting:
```bash
# Should succeed
curl -X POST http://localhost:8080/faucet -d '{"address":"LOS1test1..."}'

# Should fail (same IP)
curl -X POST http://localhost:8080/faucet -d '{"address":"LOS1test2..."}'
```
