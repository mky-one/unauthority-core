    ...
    match client.get(&url).send().await {
        Ok(response) => {
            if response.status().is_success() {
                ...
                println!("{} {}", "Timestamp:".bold(), 
                    data["timestamp"].as_u64().unwrap_or(0));
                println!("{} {}", "Proposer:".bold(), 
    ...
    match client.get(&url).send().await {
        Ok(response) => {
            if response.status().is_success() {
                ...
                println!("{} {}", "Nonce:".bold(), data["nonce"].as_u64().unwrap_or(0));
            } else {
