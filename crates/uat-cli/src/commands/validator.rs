use crate::{ValidatorCommands, print_success, print_error, print_info};
use std::path::Path;
use colored::*;
...
pub async fn handle(
    ...
    match action {
        ValidatorCommands::Stake { amount, wallet } => stake(amount, &wallet, rpc, config_dir).await?,
...
async fn stake(
    ...
    print_info(&format!("Staking {} UAT from wallet '{}'...", amount, wallet_name));
...
async fn unstake(
    ...
    println!();
    println!("{}", "⚠️  Unstake transaction not yet implemented.".yellow());
