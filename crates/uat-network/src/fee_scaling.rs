...
impl BlockBurnState {
    pub fn new(block_height: u64) -> Self {
        Self {
            ...
            total_burn_void: 0,
            burn_count: 0,
            remaining_capacity: BURN_LIMIT_PER_BLOCK_VOID,
        }
    }
    ...
    pub fn try_add_burn(&mut self, burn_amount: u128) -> Result<bool, String> {
        if burn_amount > self.remaining_capacity {
            return Err(format!(
                ...
                burn_amount, self.remaining_capacity, self.total_burn_void
            ));
        }

        self.total_burn_void += burn_amount;
    ...
    pub fn get_capacity_percentage(&self) -> f64 {
        let used = (BURN_LIMIT_PER_BLOCK_VOID - self.remaining_capacity) as f64;
        let total = BURN_LIMIT_PER_BLOCK_VOID as f64;
        (used / total) * 100.0
    }
}
...
mod tests {
    ...
    fn test_burn_limit_exceeded() {
        ...
        let result = burn_state.try_add_burn(BURN_LIMIT_PER_BLOCK_VOID + 1);
