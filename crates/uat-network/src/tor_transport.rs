/// Tor SOCKS5 Transport Layer for UAT P2P Network
///
...
///   2. Set UAT_TOR_SOCKS5=127.0.0.1:9050
///   3. Set UAT_BOOTSTRAP_NODES=<onion_addr>:<port>,<onion_addr2>:<port2>
///
...
impl TorConfig {
    ...
    pub fn from_env() -> Self {
        let socks5_proxy = std::env::var("UAT_TOR_SOCKS5")
            .ok()
            .and_then(|s| s.parse::<SocketAddr>().ok());

        let onion_address = std::env::var("UAT_ONION_ADDRESS").ok();

        let listen_port = std::env::var("UAT_P2P_PORT")
            .ok()
...
mod tests {
    ...
    fn test_load_empty_bootstrap() {
        ...
        std::env::remove_var("UAT_BOOTSTRAP_NODES");
    ...
    fn test_tor_config_defaults() {
        std::env::remove_var("UAT_TOR_SOCKS5");
        std::env::remove_var("UAT_ONION_ADDRESS");
        std::env::remove_var("UAT_P2P_PORT");
