...
/// Maximum gas per transaction (10M VOI)
...
pub fn calculate_gas_fee(
    ...
    if total_fee > MAX_GAS_PER_TX {
        return Err(format!(
            "Transaction fee {} VOI exceeds maximum {} VOI",
...
/// * `priority_tip` - Optional priority tip in VOI (0 for normal priority)
///
/// # Returns
...
/// ```ignore
/// let base_fee = 3_560;
/// let priority_tip = 10_000; // 0.0001 UAT extra for faster inclusion
...
/// * `total_fees_void` - Sum of all transaction fees in block
...
pub fn get_pending_rewards(
    ...
    rewards
        ...
        .map(|acc| acc.pending_rewards_void)
...
pub fn get_total_rewards(
    ...
    rewards
        .get(validator_address)
        .map(|acc| acc.total_rewards_void)
        .ok_or_else(|| "Validator not found in rewards".to_string())
}
...
mod tests {
    ...
    fn test_calculate_gas_fee_with_multiplier() {
        ...
        assert_eq!(fee, (1_000 + (256 * 10)) * 2); // 7,120 VOI
    }
    ...
    fn test_block_finalization() {
        ...
        let fees = vec![
            TransactionFee {
                base_fee_void: 3_560,
                priority_tip_void: 0,
                total_fee_void: 3_560,
                multiplier: 1,
                timestamp: 1000,
            },
            TransactionFee {
                base_fee_void: 5_000,
                priority_tip_void: 10_000,
                total_fee_void: 15_000,
                multiplier: 1,
                timestamp: 1001,
            },
        ];
