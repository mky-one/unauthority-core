...
pub fn load_genesis_from_config(config: &GenesisConfig) -> Result<HashMap<String, AccountState>, String> {
...
pub fn parse_uat_to_void(uat_str: &str) -> Result<u128, String> {
    ...
    if let Some(dot_pos) = trimmed.find('.') {
        ...
        let integer_part: u128 = trimmed[..dot_pos].parse()
...
pub fn validate_genesis(config: &GenesisConfig) -> Result<(), String> {
    ...
    let trimmed = config.total_supply.trim_end_matches('0').trim_end_matches('.');
    if trimmed != "21936236" {
        return Err(format!("Invalid total supply: {} (expected 21936236)", config.total_supply));
    }
