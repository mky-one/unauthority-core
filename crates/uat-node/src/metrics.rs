...
impl UatMetrics {
    /// Create new metrics registry with all UAT node metrics
    pub fn new() -> Result<Arc<Self>, Box<dyn std::error::Error>> {
        let registry = Registry::new();
        
        // Blockchain metrics
        let blocks_total = IntCounter::with_opts(
            Opts::new("uat_blocks_total", "Total number of blocks in blockchain")
        )?;
        registry.register(Box::new(blocks_total.clone()))?;
        
        let accounts_total = IntGauge::with_opts(
            Opts::new("uat_accounts_total", "Total number of accounts")
        )?;
        registry.register(Box::new(accounts_total.clone()))?;
        
        let transactions_total = IntCounter::with_opts(
            Opts::new("uat_transactions_total", "Total number of transactions processed")
        )?;
        registry.register(Box::new(transactions_total.clone()))?;
        
        let genesis_blocks_total = IntCounter::with_opts(
            Opts::new("uat_genesis_blocks_total", "Number of genesis blocks")
        )?;
        registry.register(Box::new(genesis_blocks_total.clone()))?;
        
        let send_blocks_total = IntCounter::with_opts(
            Opts::new("uat_send_blocks_total", "Number of send blocks")
        )?;
        registry.register(Box::new(send_blocks_total.clone()))?;
        
        let receive_blocks_total = IntCounter::with_opts(
            Opts::new("uat_receive_blocks_total", "Number of receive blocks")
        )?;
        registry.register(Box::new(receive_blocks_total.clone()))?;
        
        let mint_blocks_total = IntCounter::with_opts(
            Opts::new("uat_mint_blocks_total", "Number of mint blocks (PoB)")
        )?;
        registry.register(Box::new(mint_blocks_total.clone()))?;
        
        let contract_blocks_total = IntCounter::with_opts(
            Opts::new("uat_contract_blocks_total", "Number of smart contract blocks")
        )?;
        registry.register(Box::new(contract_blocks_total.clone()))?;
        
        // Database metrics
        let db_size_bytes = Gauge::with_opts(
            Opts::new("uat_db_size_bytes", "Database size in bytes")
        )?;
        registry.register(Box::new(db_size_bytes.clone()))?;
        
        let db_blocks_count = IntGauge::with_opts(
            Opts::new("uat_db_blocks_count", "Number of blocks stored in database")
        )?;
        registry.register(Box::new(db_blocks_count.clone()))?;
        
        let db_accounts_count = IntGauge::with_opts(
            Opts::new("uat_db_accounts_count", "Number of accounts stored in database")
        )?;
        registry.register(Box::new(db_accounts_count.clone()))?;
        
        let db_save_duration_seconds = Histogram::with_opts(
            HistogramOpts::new("uat_db_save_duration_seconds", "Database save operation latency")
                .buckets(vec![0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1.0])
        )?;
        registry.register(Box::new(db_save_duration_seconds.clone()))?;
        
        let db_load_duration_seconds = Histogram::with_opts(
            HistogramOpts::new("uat_db_load_duration_seconds", "Database load operation latency")
                .buckets(vec![0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1.0])
        )?;
        ...
        let consensus_rounds_total = IntCounter::with_opts(
            Opts::new("uat_consensus_rounds_total", "Total consensus rounds completed")
        )?;
        registry.register(Box::new(consensus_rounds_total.clone()))?;
        
        let consensus_failures_total = IntCounter::with_opts(
            Opts::new("uat_consensus_failures_total", "Total consensus failures")
        )?;
        registry.register(Box::new(consensus_failures_total.clone()))?;
        
        let consensus_latency_seconds = Histogram::with_opts(
            HistogramOpts::new("uat_consensus_latency_seconds", "Consensus finality latency")
                .buckets(vec![0.5, 1.0, 2.0, 3.0, 5.0, 10.0])
        )?;
        registry.register(Box::new(consensus_latency_seconds.clone()))?;
        
        let active_validators = IntGauge::with_opts(
            Opts::new("uat_active_validators", "Number of active validators")
        )?;
        registry.register(Box::new(active_validators.clone()))?;
        
        let validator_votes_total = IntCounter::with_opts(
            Opts::new("uat_validator_votes_total", "Total validator votes cast")
        )?;
        registry.register(Box::new(validator_votes_total.clone()))?;
        
        // Oracle metrics
        let oracle_submissions_total = IntCounter::with_opts(
            Opts::new("uat_oracle_submissions_total", "Total oracle price submissions")
        )?;
        registry.register(Box::new(oracle_submissions_total.clone()))?;
        
        let oracle_consensus_reached_total = IntCounter::with_opts(
            Opts::new("uat_oracle_consensus_reached_total", "Times oracle consensus reached")
        )?;
        registry.register(Box::new(oracle_consensus_reached_total.clone()))?;
        
        let oracle_outliers_total = IntCounter::with_opts(
            Opts::new("uat_oracle_outliers_total", "Oracle price outliers detected")
        )?;
        registry.register(Box::new(oracle_outliers_total.clone()))?;
        
        let oracle_btc_price_usd = Gauge::with_opts(
            Opts::new("uat_oracle_btc_price_usd", "Current BTC/USD oracle price")
        )?;
        registry.register(Box::new(oracle_btc_price_usd.clone()))?;
        
        let oracle_eth_price_usd = Gauge::with_opts(
            Opts::new("uat_oracle_eth_price_usd", "Current ETH/USD oracle price")
        )?;
        registry.register(Box::new(oracle_eth_price_usd.clone()))?;
        
        // PoB Distribution metrics
        let pob_burns_total = IntCounter::with_opts(
            Opts::new("uat_pob_burns_total", "Total PoB burn events")
        )?;
        registry.register(Box::new(pob_burns_total.clone()))?;
        
        let pob_minted_uat = Counter::with_opts(
            Opts::new("uat_pob_minted_uat", "Total UAT minted via PoB")
        )?;
        registry.register(Box::new(pob_minted_uat.clone()))?;
        
        let pob_burned_btc = Counter::with_opts(
            Opts::new("uat_pob_burned_btc", "Total BTC burned (in satoshis)")
        )?;
        registry.register(Box::new(pob_burned_btc.clone()))?;
        
        let pob_burned_eth = Counter::with_opts(
            Opts::new("uat_pob_burned_eth", "Total ETH burned (in wei)")
        )?;
        registry.register(Box::new(pob_burned_eth.clone()))?;
        
        let pob_remaining_supply = Gauge::with_opts(
            Opts::new("uat_pob_remaining_supply", "Remaining UAT supply for PoB distribution")
        )?;
        registry.register(Box::new(pob_remaining_supply.clone()))?;
        
        // Network metrics
        let connected_peers = IntGauge::with_opts(
            Opts::new("uat_connected_peers", "Number of connected P2P peers")
        )?;
        registry.register(Box::new(connected_peers.clone()))?;
        
        let p2p_messages_received_total = IntCounter::with_opts(
            Opts::new("uat_p2p_messages_received_total", "Total P2P messages received")
        )?;
        registry.register(Box::new(p2p_messages_received_total.clone()))?;
        
        let p2p_messages_sent_total = IntCounter::with_opts(
            Opts::new("uat_p2p_messages_sent_total", "Total P2P messages sent")
        )?;
        registry.register(Box::new(p2p_messages_sent_total.clone()))?;
        
        let p2p_bytes_received_total = Counter::with_opts(
            Opts::new("uat_p2p_bytes_received_total", "Total bytes received via P2P")
        )?;
        registry.register(Box::new(p2p_bytes_received_total.clone()))?;
        
        let p2p_bytes_sent_total = Counter::with_opts(
            Opts::new("uat_p2p_bytes_sent_total", "Total bytes sent via P2P")
        )?;
        registry.register(Box::new(p2p_bytes_sent_total.clone()))?;
        
        // API metrics
        let api_requests_total = IntCounter::with_opts(
            Opts::new("uat_api_requests_total", "Total REST API requests")
        )?;
        registry.register(Box::new(api_requests_total.clone()))?;
        
        let api_errors_total = IntCounter::with_opts(
            Opts::new("uat_api_errors_total", "Total REST API errors")
        )?;
        registry.register(Box::new(api_errors_total.clone()))?;
        
        let api_request_duration_seconds = Histogram::with_opts(
            HistogramOpts::new("uat_api_request_duration_seconds", "REST API request latency")
                .buckets(vec![0.001, 0.01, 0.05, 0.1, 0.5, 1.0, 5.0])
        )?;
        registry.register(Box::new(api_request_duration_seconds.clone()))?;
        
        let grpc_requests_total = IntCounter::with_opts(
            Opts::new("uat_grpc_requests_total", "Total gRPC requests")
        )?;
        registry.register(Box::new(grpc_requests_total.clone()))?;
        
        let grpc_errors_total = IntCounter::with_opts(
            Opts::new("uat_grpc_errors_total", "Total gRPC errors")
        )?;
        registry.register(Box::new(grpc_errors_total.clone()))?;
        
        // Rate limiter metrics
        let rate_limit_rejections_total = IntCounter::with_opts(
            Opts::new("uat_rate_limit_rejections_total", "Total rate limit rejections")
        )?;
        registry.register(Box::new(rate_limit_rejections_total.clone()))?;
        
        let rate_limit_active_ips = IntGauge::with_opts(
            Opts::new("uat_rate_limit_active_ips", "Number of IPs being tracked by rate limiter")
        )?;
        registry.register(Box::new(rate_limit_active_ips.clone()))?;
        
        // Slashing metrics
        let slashing_events_total = IntCounter::with_opts(
            Opts::new("uat_slashing_events_total", "Total slashing events")
        )?;
        registry.register(Box::new(slashing_events_total.clone()))?;
        
        let slashing_total_amount = Counter::with_opts(
            Opts::new("uat_slashing_total_amount", "Total UAT slashed (in VOI)")
        )?;
        registry.register(Box::new(slashing_total_amount.clone()))?;
        
        // Smart contract metrics
        let contracts_deployed_total = IntCounter::with_opts(
            Opts::new("uat_contracts_deployed_total", "Total smart contracts deployed")
        )?;
        registry.register(Box::new(contracts_deployed_total.clone()))?;
        
        let contract_executions_total = IntCounter::with_opts(
            Opts::new("uat_contract_executions_total", "Total contract executions")
        )?;
        registry.register(Box::new(contract_executions_total.clone()))?;
        
        let contract_gas_used_total = Counter::with_opts(
            Opts::new("uat_contract_gas_used_total", "Total gas consumed by contracts")
        )?;
    ...
    pub fn update_blockchain_metrics(&self, ledger: &uat_core::Ledger) {
        ...
        for block in ledger.blocks.values() {
            match block.block_type {
                uat_core::BlockType::Send => send_count += 1,
        ...
        // PoB distribution metrics
        self.pob_remaining_supply.set(ledger.distribution.remaining_supply as f64);
    }
...
mod tests {
    ...
    fn test_metrics_creation() {
        let metrics = UatMetrics::new().unwrap();
    ...
    fn test_metrics_export() {
        ...
        assert!(output.contains("uat_blocks_total"));
        assert!(output.contains("uat_accounts_total"));
    ...
    fn test_histogram_observe() {
        ...
        assert!(export.contains("uat_consensus_latency_seconds"));
    }
}
