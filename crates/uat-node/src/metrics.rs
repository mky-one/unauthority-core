...
impl UatMetrics {
    ...
    pub fn new() -> Result<Arc<Self>, Box<dyn std::error::Error>> {
        ...
        let accounts_total =
            IntGauge::with_opts(Opts::new("uat_accounts_total", "Total number of accounts"))?;
        ...
        let send_blocks_total =
            IntCounter::with_opts(Opts::new("uat_send_blocks_total", "Number of send blocks"))?;
        ...
        // Database metrics
        let db_size_bytes =
            Gauge::with_opts(Opts::new("uat_db_size_bytes", "Database size in bytes"))?;
        ...
        // PoB Distribution metrics
        let pob_burns_total =
            IntCounter::with_opts(Opts::new("uat_pob_burns_total", "Total PoB burn events"))?;
        registry.register(Box::new(pob_burns_total.clone()))?;

        let pob_minted_uat =
            Counter::with_opts(Opts::new("uat_pob_minted_uat", "Total UAT minted via PoB"))?;
        ...
        let api_errors_total =
            IntCounter::with_opts(Opts::new("uat_api_errors_total", "Total REST API errors"))?;
        registry.register(Box::new(api_errors_total.clone()))?;

        let api_request_duration_seconds = Histogram::with_opts(
            HistogramOpts::new(
                "uat_api_request_duration_seconds",
        ...
        let grpc_requests_total =
            IntCounter::with_opts(Opts::new("uat_grpc_requests_total", "Total gRPC requests"))?;
        registry.register(Box::new(grpc_requests_total.clone()))?;

        let grpc_errors_total =
            IntCounter::with_opts(Opts::new("uat_grpc_errors_total", "Total gRPC errors"))?;
