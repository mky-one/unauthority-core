//! UAT Crypto FFI — C-ABI bridge for Flutter wallet (dart:ffi)
//!
...
///
/// # Arguments
/// - `pk_out`: Pre-allocated buffer for public key (must be >= uat_public_key_bytes())
/// - `pk_capacity`: Buffer size
/// - `sk_out`: Pre-allocated buffer for secret key (must be >= uat_secret_key_bytes())
...
/// - -2: buffer too small
#[no_mangle]
pub extern "C" fn uat_generate_keypair(
    pk_out: *mut u8,
    pk_capacity: i32,
    sk_out: *mut u8,
    sk_capacity: i32,
...
/// "UAT" + Base58( 0x4A | BLAKE2b-160(pubkey) | SHA256²(payload)[0..4] )
...
/// - -2: buffer too small
#[no_mangle]
pub extern "C" fn uat_public_key_to_address(
    public_key: *const u8,
    pk_len: i32,
    address_out: *mut u8,
    addr_capacity: i32,
    ...
    // 6. "UAT" prefix
    let full_address = format!("UAT{}", base58_addr);
...
/// 1 = valid, 0 = invalid
#[no_mangle]
pub extern "C" fn uat_validate_address(
    address: *const u8,
    addr_len: i32,
) -> i32 {
    ...
    // Must start with "UAT"
    if !addr_str.starts_with("UAT") {
        return 0;
    }

    // Decode Base58 (remove "UAT" prefix)
...
#[no_mangle]
pub extern "C" fn uat_bytes_to_hex(
    input: *const u8,
    input_len: i32,
    hex_out: *mut u8,
    hex_capacity: i32,
...
#[no_mangle]
pub extern "C" fn uat_hex_to_bytes(
    hex_in: *const u8,
    hex_len: i32,
    bytes_out: *mut u8,
    bytes_capacity: i32,
...
mod tests {
    ...
    fn test_keygen_sign_verify() {
        ...
        let result = uat_generate_keypair(
    ...
    fn test_address_derivation() {
        ...
        let addr_len = uat_public_key_to_address(
            pk.as_ptr(), pk_size as i32,
            addr.as_mut_ptr(), 64,
        );
        ...
        assert!(address.starts_with("UAT"), "Address must start with UAT: {}", address);
        println!("Generated address: {}", address);

        // Validate
        let valid = uat_validate_address(
    ...
    fn test_address_consistency_with_backend() {
        ...
        let len1 = uat_public_key_to_address(
            pk.as_ptr(), pk_size as i32,
            addr1.as_mut_ptr(), 64,
        );
        ...
        let mut address_bytes = payload;
        address_bytes.extend_from_slice(checksum);
        let manual_address = format!("UAT{}", bs58::encode(&address_bytes).into_string());
