#!/bin/bash
# UNAUTHORITY (LOS) - VALIDATOR SETUP SCRIPT v2.0
...
echo ""
echo "╔════════════════════════════════════════════════════════════╗"
echo "║  UNAUTHORITY VALIDATOR SETUP                              ║"
echo "║  Setting up 3 Bootstrap Validators with unique addresses  ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo ""
...
echo "📌 Extracting validator addresses from genesis..."
...
VALIDATOR_1_STAKE=$(jq -r '.bootstrap_nodes[0].stake_cil' "$GENESIS_CONFIG")
VALIDATOR_2_STAKE=$(jq -r '.bootstrap_nodes[1].stake_cil' "$GENESIS_CONFIG")
VALIDATOR_3_STAKE=$(jq -r '.bootstrap_nodes[2].stake_cil' "$GENESIS_CONFIG")
...
if [ -z "$VALIDATOR_1_ADDR" ] || [ "$VALIDATOR_1_ADDR" == "null" ]; then
    echo "❌ ERROR: Failed to extract bootstrap node addresses from genesis!"
    exit 1
fi
...
echo "📌 Creating validator node directories..."
...
echo "📌 Generating validator.toml configs..."
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# UNAUTHORITY (LOS) - VALIDATOR CONFIGURATION v2.0
...
EOF
    ...
    echo "   ✓ Created: $DIR/validator.toml"
...
echo "📌 Copying genesis config to validators..."

for i in 1 2 3; do
    cp "$GENESIS_CONFIG" "${NODE_DATA_DIR}/validator-$i/genesis_config.json"
    echo "   ✓ Copied to validator-$i"
done
...
export LOS_VALIDATOR_ADDRESS="$ADDR"
export LOS_NODE_ID="validator-$i"
export LOS_DATA_DIR="${NODE_DATA_DIR}/validator-$i"
EOF

    echo "   ✓ Created .env for validator-$i"
done
...
echo ""
echo "⚠️  SECURITY:"
echo "   • Private keys stored in node_data/validator-*/private_key.hex"
echo "   • Never commit private_key.hex to git!"
echo "   • Backup keys to secure cold storage"
echo ""
