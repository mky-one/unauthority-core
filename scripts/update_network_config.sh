...
if [[ ! -f "$HOSTNAME_FILE" ]]; then
  echo "❌ No hidden service found at $HOSTNAME_FILE"
  echo "   Run: brew services restart tor  (with HiddenServiceDir in torrc)"
  exit 1
fi

ONION_ADDR=$(cat "$HOSTNAME_FILE" | tr -d '[:space:]')
echo "🧅 Current .onion: $ONION_ADDR"

CONFIG_TEMPLATE=$(cat <<ENDJSON
{
  "version": 1,
  "comment": "Bootstrap node addresses — AUTO-GENERATED by scripts/update_network_config.sh. DO NOT hardcode in source.",
  "testnet": {
    "bootstrap_nodes": [
      {
        "name": "validator-1",
...
ENDJSON2
echo "✅ Updated: testnet-tor-info.json"

echo ""
echo "🎉 All config files updated with: $ONION_ADDR"
