...
async fn test_zero_trust_sync() {
    ...
    for (i, node) in nodes.iter().enumerate() {
        ...
        mint_hashes.push(result.unwrap());
...
async fn test_financial_precision() {
    ...
    let mint_hash = ledger.process_block(&mint_block).expect("Mint failed");
...
async fn test_node_recovery() {
    ...
    let mint_hash = ledger.process_block(&mint).expect("Mint failed");
...
// ============================================================================
// TEST 6: VALIDATOR REWARDS (Epoch, √Stake, Halving)
// ============================================================================
// All math integer via isqrt(). No f64 anywhere.
#[tokio::test]
async fn test_validator_rewards_epoch() {
    println!("\n🧪 TEST 6: Validator Rewards (Epoch + √Stake)");
    println!("================================================\n");
    ...
    println!(
        "  ✅ √stake weighting: V0(1000LOS)={} CIL, V3(10000LOS)={} CIL",
        r0, r3
    );
...
// ============================================================================
// TEST 8: QUADRATIC VOTING & ANTI-WHALE
// ============================================================================
// √stake voting power prevents plutocracy. Fee scaling deters spam.
#[tokio::test]
async fn test_quadratic_voting_and_antiwhale() {
    println!("\n🧪 TEST 8: Quadratic Voting & Anti-Whale");
    println!("==========================================\n");
    ...
    // 10x stake should give ~3.16x power (√10 ≈ 3.16), not 10x
    // Integer: √10000 / √1000 = 100 / 31 ≈ 3.22
    if powers[0] > 0 {
        let ratio_10x = (powers[1] * 100) / powers[0]; // basis points-like
        assert!(
            ratio_10x < 400 && ratio_10x > 250,
        ...
        assert!(
            ratio_100x < 1200 && ratio_100x > 800,
    ...
    let config = los_core::anti_whale::AntiWhaleConfig {
        max_tx_per_block: 5,
    ...
    // First tx: base fee
    let fee1 = engine.estimate_fee("LOS_spammer", BASE_FEE_CIL as u64);
    // Register multiple txs from same address → escalating fees
    let _ = engine.register_transaction("LOS_spammer".to_string(), BASE_FEE_CIL as u64);
    let fee2 = engine.estimate_fee("LOS_spammer", BASE_FEE_CIL as u64);
    let _ = engine.register_transaction("LOS_spammer".to_string(), BASE_FEE_CIL as u64);
    let fee3 = engine.estimate_fee("LOS_spammer", BASE_FEE_CIL as u64);

    println!("  Fee escalation: f1={}, f2={}, f3={}", fee1, fee2, fee3);
    assert!(fee2 >= fee1, "Fee must not decrease with repeated txs");
    assert!(fee3 >= fee2, "Fee must escalate");
    println!("  ✅ Anti-whale fee scaling: monotonically increasing");

    // 4. Whale detection
    let total_supply = 21_936_236u64 * (CIL_PER_LOS as u64 / 1_000_000); // scaled down to fit u64
    let whale_stake = total_supply / 50; // 2% = whale
    let normal_stake = total_supply / 10_000; // 0.01% = not whale
    assert!(engine.is_whale(whale_stake, total_supply));
    assert!(!engine.is_whale(normal_stake, total_supply));
    println!("  ✅ Whale detection: 2% = whale, 0.01% = not whale");

    println!("\n  📊 Quadratic voting + anti-whale: verified");
}
...
async fn test_throughput_benchmark() {
    ...
    let mut prev_hash = ledger.process_block(&mint).expect("Mint failed");
